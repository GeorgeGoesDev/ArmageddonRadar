...
function ErrorState({ message, onRetry, onDemo }: { message: string; onRetry: () => void; onDemo: () => void }) {
  ...
  return (
    <View className="px-4 py-10 items-center">
      ...
      <Text className="mt-4 text-center text-base font-semibold" style={{ color: colors.textPrimary }}>{t('dashboard.errorTitle')}</Text>
      <Text className="mt-2 text-center text-xs" style={{ color: colors.textMuted }}>{message}</Text>
      <View className="flex-row mt-5">
        <Pressable onPress={onRetry} className="rounded-xl px-5 py-3 mr-3" style={{ backgroundColor: colors.accentBlue }}><Text className="font-bold" style={{ color: colors.spaceBlack }}>{t('common.retry')}</Text></Pressable>
        <Pressable onPress={onDemo} className="rounded-xl px-5 py-3" style={{ borderWidth: 1.5, borderColor: colors.accentBlue }}><Text className="font-bold" style={{ color: colors.accentBlue }}>{t('dashboard.useDemoData')}</Text></Pressable>
      </View>
    </View>
  );
}
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: colors.spaceBlack }} edges={['top']}>
      ...
      {isLoading ? (
        ...
        <ErrorState message={error?.message ?? t('dashboard.unknownError')} onRetry={() => refetch()} onDemo={() => setUseMock(true)} />
      ) : (
        <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={isRefetching} onRefresh={() => refetch()} tintColor={colors.accentBlue} />}>
          ...
          {closest ? (
            <>
              ...
              <Text className="px-4 mt-7 mb-2 text-xs uppercase tracking-widest" style={{ color: colors.accentBlue }}>{t('dashboard.liveRadar')}</Text>
              ...
              <View className="flex-row items-center justify-between px-4 mt-7 mb-2">
                <Text className="text-xs uppercase tracking-widest" style={{ color: colors.accentBlue }}>{t('dashboard.activeTracking', { count: visibleList.length })}</Text>
                {useMock && <Text className="text-[10px]" style={{ color: colors.textMuted }}>{t('dashboard.demoDataBadge')}</Text>}
              </View>
              <View className="mb-2"><ListControlsBar controls={controls} onChange={setControls} /></View>

              <View className="px-4 mt-2">
                {visibleList.length === 0 ? (
                  <Text className="text-center text-xs py-8" style={{ color: colors.textMuted }}>{t('dashboard.noMatch')}</Text>
                ) : (
                  visibleList.map((a) => (
            ...
            <View className="px-4 py-16 items-center">
              ...
              <Text className="mt-4 text-lg font-bold" style={{ color: colors.textPrimary }}>{t('dashboard.clearSkies')}</Text>
              <Text className="mt-2 text-center text-xs" style={{ color: colors.textMuted }}>{t('dashboard.clearSkiesBody')}</Text>
            </View>
          )}
        </ScrollView>
      )}
